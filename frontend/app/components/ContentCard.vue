  ...
  <article class="group overflow-hidden rounded-[24px] border border-[var(--line)] bg-[#fbfaf7] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_45px_rgba(50,43,32,.08)]">
    <NuxtLink :to="`/content/${post.id}`" class="relative block aspect-[4/3] overflow-hidden bg-[#ded9cf]">
      ...
      <div class="absolute left-5 right-5 top-5 flex items-center justify-between">
        ...
        <span class="rounded-full bg-[#f3ebe1] px-3 py-1.5 text-[12px] font-semibold text-[#8b402a]">{{ post.performance_ratio.toFixed(1) }}× average</span>
      </div>
    ...
    <div class="p-5 md:p-6">
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0 flex-1"><p class="truncate text-sm font-medium">@{{ post.creator.username }}</p><p class="text-[11px] text-[#8f8b83]">{{ compactNumber(post.creator.followers) }} followers · {{ relativeDate(post.published_at) }}</p></div>
        <div class="text-right text-[11px] text-[#77736c]"><p>{{ compactNumber(post.views) }} views</p><p>{{ compactNumber(post.likes) }} likes</p></div>
      </div>
      ...
      <div class="mt-5 border-l-2 border-[#c85234]/55 pl-4"><p class="text-[10px] font-semibold uppercase tracking-[.12em] text-[#918d85]">Why Personal recommends this</p><p class="mt-1.5 text-[13px] leading-5 text-[#5f5c56]">{{ post.why_recommended || post.why_it_works }}</p></div>

      <div class="mt-6 flex items-center gap-2">
        <button class="rounded-full border border-[#d9d5cc] px-3.5 py-2 text-xs transition hover:bg-[#f0ede6]" @click="$emit('save', post)">{{ post.is_saved ? 'Saved ✓' : 'Save' }}</button>
        <button class="rounded-full px-3 py-2 text-xs text-[#8b877f] hover:text-[#33312d]" @click="$emit('dismiss', post)">Not for me</button>
        <button class="ml-auto inline-flex items-center gap-2 rounded-full bg-[#1d1d1b] px-4 py-2.5 text-xs font-medium text-white transition hover:bg-black" @click="$emit('remix', post)">Remix for me <AppIcon name="arrow" :size="14" /></button>
      </div>
    </div>
  </article>
</template>
