    ...
    <div class="mx-auto grid max-w-[1200px] gap-12 md:grid-cols-[minmax(0,340px)_1fr] md:gap-20">
      ...
      <div data-reveal class="border-t border-[var(--b-line)]">
        <details v-for="key in KEYS" :key="key" class="faq group border-b border-[var(--b-line)]">
          <summary class="b-focus flex cursor-pointer list-none items-start justify-between gap-8 py-6 text-[17px] leading-[1.45] tracking-[-.01em] transition-colors hover:text-[var(--b-stone)] md:text-[18.5px]">
            {{ $t(`landing.faq.items.${key}.q`) }}
            <span
              class="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[var(--b-line)] text-[var(--b-stone)] transition-all duration-300 group-open:rotate-45 group-open:border-[var(--b-black)] group-open:bg-[var(--b-black)] group-open:text-[var(--b-ivory)]"
              aria-hidden="true"
            >
              <AppIcon name="plus" :size="15" />
            </span>
          </summary>
          <p class="max-w-2xl border-l border-[var(--b-line)] pb-1 pl-5 text-[15.5px] leading-[1.7] text-[var(--b-stone)]">
