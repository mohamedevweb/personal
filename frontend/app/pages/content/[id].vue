  ...
  <main v-if="post" class="mx-auto max-w-[1180px] px-5 pb-16 pt-2 md:px-8">
    ...
    <div class="mt-6 grid gap-10 lg:grid-cols-[.88fr_1.12fr]">
      <section class="lg:sticky lg:top-8 lg:self-start">
        <div class="relative aspect-[4/5] overflow-hidden rounded-[22px] bg-[#e6e6e2]"><img :src="post.thumbnail_url || ''" :alt="post.hook" class="h-full w-full object-cover"><div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/5 to-transparent"/><div class="absolute inset-x-6 bottom-6"><span class="rounded-full bg-white/15 px-3 py-1.5 text-[11px] text-white backdrop-blur">{{ post.format }}</span><h1 class="mt-4 text-[28px] font-medium leading-[1.12] tracking-[-.03em] text-white">{{ post.hook }}</h1></div></div>
      ...
      <section>
        ...
        <div class="mt-5 inline-flex items-baseline gap-2 rounded-2xl bg-[var(--accent-soft)] px-5 py-4 text-[#8a6413]"><span class="font-serif text-4xl">{{ post.performance_ratio.toFixed(1) }}×</span><span class="text-xs">{{ $t('content.usualPerformance') }}</span></div>
        ...
        <div class="mt-8 rounded-[20px] border border-[var(--line)] bg-[var(--surface)] p-6">
          <h2 class="font-serif text-2xl">{{ $t('content.makeItYours') }}</h2><p class="mt-2 text-sm text-[var(--muted)]">{{ $t('content.makeItYoursCopy') }}</p>
          <div class="mt-5 flex flex-wrap gap-2"><button v-for="item in ['reel','carousel','caption']" :key="item" class="rounded-full border px-4 py-2 text-xs capitalize" :class="format === item ? 'border-[var(--ink)] bg-[var(--ink)] text-white' : 'border-[var(--line)]'" @click="format = item as any">{{ item === 'caption' ? $t('content.captionOption') : item }}</button></div>
          ...
          <button class="mt-6 flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--ink)] text-sm font-medium text-white disabled:opacity-60" :disabled="generating" @click="createRemix">{{ generating ? $t('content.creating') : $t('content.remixForMe') }} <AppIcon name="sparkles" :size="16" /></button>
        </div>
      </section>
    </div>
  </main>
